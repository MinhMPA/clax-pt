...
In all 11 cases, the pattern was the same: the oracle test produced a clear numerical discrepancy, the agent compared intermediate quantities against reference data to localize the error, and the fix was a direct transcription correction.
...
The three human-essential interventions were thus: (i)~the Gauss--Legendre architectural redesign that unblocked all six redshift-space multipoles (\cref{sec:wall}), (ii)~the rejection of the fudge factor $\alpha=0.27$ as physically unmotivated, and (iii)~the identification of the correct anisotropic tree-level formula from an independent implementation. Items (ii) and (iii) occurred in the same debugging episode but represent distinct cognitive acts: rejecting a wrong answer versus finding the right one.
...
The division of labor that emerged---classified retrospectively from the CHANGELOG and git history, with each session defined as a single Claude Code invocation typically lasting 15--45 minutes---was: approximately 80\% of sessions were autonomous oracle-following (transcription, debugging, implementation), approximately 10\% were engineering tasks where the agent worked independently given a well-specified architecture, and approximately 10\% required irreplaceable human judgment (architectural decisions and fudge-factor rejection). The binding constraint on code quality was the supervision protocol, not the model's raw capability.
For context, the reference code \classpt was developed over approximately eight years (2,112 commits since 2018) by a team of expert theoretical physicists who both derived the theoretical framework and implemented it in C~\citep{chudaykin2021}; a comparable human reimplementation of the same algorithm in a modern framework typically requires 6--9 months of focused effort for a pair of experienced researchers, based on the authors' experience with related projects.
The physicist--agent collaboration compressed this to 12 days, with the physicist contributing an estimated 20--40 hours of supervision rather than months of hands-on coding.
The speedup reflects the agent's strength at the 80\% of work that is transcription and debugging against a known oracle; the physicist's irreplaceable 10\% contribution---which would have taken the same time regardless of implementation speed---was the binding constraint on correctness, not on throughput.
