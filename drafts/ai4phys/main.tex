...
Solid arrows show data flow; dashed orange arrows show where \class and \classpt reference data feed into the oracle test suite.
Blue dashed arrows indicate reverse-mode AD gradient flow (\texttt{jax.grad}) from outputs back through the full pipeline to \texttt{CosmoParams}.
Each module is independently tested against reference data before the next is implemented.
The Einstein--Boltzmann system is integrated with Kvaerno5 (ESDIRK) or Rodas5 (Rosenbrock) solvers via Diffrax.}
\label{fig:pipeline}
\end{figure*}
