...
The standard model of cosmology is tested by comparing theoretical predictions against observations from galaxy redshift surveys such as the Dark Energy Spectroscopic Instrument~\citep{desi2025v, desi2025vii} and cosmic microwave background experiments including the Planck satellite~\citep{planck2020}.
...
Gradient-based inference libraries such as \textsc{NumPyro} \citep{phan2019}, \textsc{BlackJAX} \citep{cabezas2024}, and normalizing-flow-augmented MCMC \citep{gabrie2022} are mature, but the missing ingredient is a differentiable cosmological forward model that can be passed to them.

The industry standard Boltzmann codes---\class \citep{lesgourgues2011} and \textsc{CAMB} \citep{lewis2000}---are written in C and Fortran, are not differentiable, and are not GPU-native.
