...
% ===========================================================================
\begin{abstract}
Gradient-based inference methods such as Hamiltonian Monte Carlo and simulation-based inference require differentiable forward models, yet no existing code provides full CMB spectra plus one-loop galaxy power spectra with exact automatic differentiation on GPU.
We address this gap with \clax and \claxpt, fully differentiable \jax reimplementations of the \class Boltzmann solver and its one-loop perturbation theory extension \classpt, developed via oracle-guided agentic development: AI agents iterate against reference data from \class and \classpt, autonomously resolving implementation bugs and escalating to human review only when oracle feedback stalls.
Across 57 Claude Code sessions spanning 12 days, agents autonomously resolved 38 of 40 documented bugs (95\%) without human input.
...
\clax achieves $< 0.1\%$ CMB accuracy at $\ell \leq 2000$; \claxpt achieves $< 0.7\%$ galaxy power spectrum accuracy for monopole and quadrupole spectra at $k < 0.3\;h/$Mpc.
...
The standard model of cosmology is tested by comparing theoretical predictions against observations from galaxy surveys such as DESI \citep{desi2025}, Euclid, and CMB experiments including Planck.
This comparison increasingly demands gradient-based inference: exact Fisher matrices, HMC sampling \citep{neal2011}, and simulation-based inference \citep{cranmer2020} all benefit from differentiable forward models.
Libraries such as NumPyro \citep{phan2019}, BlackJAX \citep{cabezas2024}, and flowMC \citep{gabrie2022} provide state-of-the-art gradient-based samplers, but the missing ingredient is a differentiable cosmological forward model that can be passed to them.
...
Several \jax implementations exist but cover only subsets of the problem: \discoeb \citep{hahn2023} computes the nonlinear matter power spectrum $\Pk$ but not CMB spectra or galaxy clustering observables; \abcmb \citep{zhou2026} implements the CMB pipeline including lensing and massive neutrinos but does not include perturbation theory for galaxy surveys; \textsc{jax-cosmo} \citep{lemos2023} computes the nonlinear matter power spectrum but not the full Boltzmann hierarchy; and SymBoltz.jl \citep{hermansen2024} provides full CMB spectra in Julia but without GPU support.
...
We describe the development of \clax and \claxpt---fully differentiable \jax reimplementations of the \class and \classpt pipelines---and the oracle-guided agentic methodology used to build them.
Using \class v3.3.4 and \classpt \citep{chudaykin2021} as ground truth, we find a clear boundary: agents handle implementation bugs (convention errors, algorithmic transcription, coefficient verification) autonomously and reliably, but fail at two classes of problem that require human physics judgment: (1) recognizing that an architecture is structurally incompatible with the intended physics, and (2) detecting that a numerically effective patch satisfies all oracle tests at the calibration point while encoding incorrect physics.
...
An exact Fisher matrix for parameter inference requires $\partial \Cl / \partial \theta_i$; HMC requires the full gradient $\nabla_\theta \log P(\boldsymbol{\theta} | \mathrm{data})$; both are expensive to approximate by finite differences and exact only with automatic differentiation (AD).
\clax enables \texttt{jax.grad($\Cl$)(params)} for the first time in the CMB context, and \claxpt extends this to one-loop galaxy power spectrum multipoles.

\subsection{AI-assisted scientific code development}

The oracle-based methodology we employ is inspired by the AI C compiler project \citep{carlini2026}, in which 16 parallel Claude Code agents produced a 100k-line Rust C compiler over $\approx$2000 sessions, using GCC as an oracle.
...
\begin{equation*}
\text{CosmoParams} \to \text{BG} \to \text{TH} \to \text{PT} \to \text{PR} \to \text{TR} \to \text{HR} \to \text{LE}
\end{equation*}
where BG is background cosmology, TH is thermodynamics/recombination, PT is perturbation theory (Einstein--Boltzmann hierarchy), PR is the primordial spectrum, TR is transfer functions, HR is harmonic ($\Cl$) integration, and LE is CMB lensing via Wigner $d$-functions.
...
\clax was developed primarily by the first author over $\sim$6 days using Claude Code (Opus 4.6), producing $\sim$10,000 lines of \jax code across $\geq$100 agent sessions.
\claxpt was developed by the second author over 12 days and 57 logged sessions, producing $\sim$2,100 lines.
...
Quadrupoles ($\ell = 2$) agree to $< 0.7\%$ for matter and $< 1.1\%$ for galaxies (the galaxy quadrupole has a known residual error in a tree-level bias term).
...
$P_{mm}^{(\ell=2)}$, $P_{gg}^{(\ell=2)}$ & 0.70\%, 1.11\% & rel. \\
...
An alternative Rodas5 Rosenbrock solver \citep{dimarzio1993}, following \discoeb, avoids Newton iteration and offers $\sim$3--5$\times$ speedup on the perturbation ODE.
...
Finite-difference gradients require $2d$ forward evaluations (centered differences); AD requires a single forward pass plus one backward pass regardless of $d$.
For \clax with $d = 6$ standard $\Lambda$CDM parameters, this represents a $\sim$12$\times$ reduction in gradient cost.
For extended models ($d = 10$--$20$ including neutrino masses, dark energy, bias parameters, and counterterms), the advantage grows to 20--40$\times$.
Combined with the 34\,s forward evaluation of \texttt{fit\_cl}, this makes gradient-based sampling with exact theory predictions feasible for the first time in full-shape galaxy clustering analysis.
...
\discoeb \citep{hahn2023} implements the nonlinear matter power spectrum in \jax but does not compute CMB spectra ($\Cl$) or galaxy clustering observables.
...
SymBoltz.jl \citep{hermansen2024} provides a full Boltzmann solver in Julia with automatic differentiation but without GPU support.
\textsc{jax-cosmo} \citep{lemos2023} computes the nonlinear matter power spectrum but not the full Boltzmann hierarchy and not CMB spectra.
\clax is the first \jax code to provide full CMB spectra plus one-loop galaxy power spectra with exact automatic differentiation and GPU execution.
...
The codes achieve $< 0.1\%$ CMB accuracy, $< 0.7\%$ galaxy power spectrum accuracy, with automatic differentiation gradients verified to 0.15\% and GPU execution at 34\,s/step on V100.
