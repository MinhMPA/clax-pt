...
Standard Boltzmann solvers for CMB angular power spectrum, e.g., \class \citep{lesgourgues2011}, and effective-field-theory (EFT) codes for one-loop EFT galaxy power spectrum, e.g., \classpt, are written in C (or Fortran), are not differentiable, and are not GPU-native.
...
Two branches produce CMB angular power spectra (left, via line-of-sight integration and lensing) and galaxy power spectrum multipoles (right, via one-loop EFT perturbation theory).
...
\Cref{tab:pk_accuracy} summarizes accuracy for the linear matter power spectrum $\Pk$ (\clax vs.\ \class) and one-loop EFT galaxy power spectra (\claxpt vs.\ \classpt at $z = 0.38$, the BOSS effective redshift).
...
\midrule
\multicolumn{4}{l}{\textit{One-loop EFT (\claxpt vs.\ \classpt, $z = 0.38$)}} \\
...
The EFT of large-scale structure \citep{perko2016, damico2020} provides a systematic expansion for the galaxy power spectrum multipoles.
