...
class TestClppHalofitRatio:
    ...
    def test_ratio_at_low_l(self, cl_pp_results, class_reference):
        """NL/linear ratio matches CLASS within 5% for l <= 500.

        The new path applies sqrt(R(k, z)) directly to the source at each
        (k, tau) — the same construction CLASS uses — so we expect tighter
        agreement than the previous Poisson-Limber path.
        """
        ...
        for l_val in [100, 200, 500]:
            ...
            assert rel_err < 0.05, (
                f"l={l_val}: NL correction err={rel_err:.1%} exceeds 5%")
